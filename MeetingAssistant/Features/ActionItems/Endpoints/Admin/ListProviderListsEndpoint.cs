...
namespace MeetingAssistant.Features.ActionItems.Endpoints.Admin
{
    ...
    {
        [HttpGet("{provider}/projects/{projectId}/lists")]
        public async Task<IActionResult> ListProviderLists(
            Guid organizationId,
            string provider,
            string projectId,
            CancellationToken cancellationToken = default)
        {
            ...
            var result = await _integrationAdminService.GetListsAsync(organizationId, providerEnum, projectId, cancellationToken);
